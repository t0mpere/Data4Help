...
router.post('/makeRequest',(req,res) =>{
    ...
    //at least one parameter has to be requested, otherwise the query is useless
    if(!(params.avg_bpm === 'true' || params.avg_bp_min === 'true' || params.avg_bp_max === 'true'))
        check = false;
    ...
    if(params.age_to < params.age_from || params.age_to < 0 || params.age_from < 0)
        check = false;
    ...
    if(params.periodical !== '0')
        ...
        let day_from = parseInt(date_from[0], 10);
        let month_from = parseInt(date_from[1], 10);
        let year_from = parseInt(date_from[2], 10);
        let day_to = parseInt(date_to[0], 10);
        let month_to = parseInt(date_to[1], 10);
        let year_to = parseInt(date_to[2], 10);
        ...
        params.date_from = new Date(date_from);
        params.date_to = new Date(date_to);
    }
